<template>
    <div class="structure-hotspot-tab">
        <el-card class="panel-card" shadow="hover">
            <template #header>
                <div class="card-header">结构相似热点组件</div>
            </template>

            <el-table :data="rows" border row-key="structure_cluster_id" max-height="46vh" highlight-current-row
        ...
        <section class="detail-section">
            <div class="section-title">结构相似热点组件详情</div>
            <el-card v-for="row in rows" :key="row.structure_cluster_id" class="panel-card detail-item-card"
                shadow="hover">
                <template #header>
...
const initSelection = async () => {
    ...
    const matched = props.rows.find((row) => row.structure_cluster_id === currentId)
